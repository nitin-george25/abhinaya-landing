...
import { Reveal, StaggerGroup, revealItem, motion } from "./motion";
...
export default function Gallery() {
  ...
  return (
    <section id="gallery" style={{ maxWidth: 1280, margin: "0 auto", padding: "96px 32px" }}>
      <Reveal><SectionHeader eyebrow="The experience" title="Inside Abhinaya" /></Reveal>
      <StaggerGroup as="div" className="gallery-grid" stagger={0.09}>
        {shots.map((s) => (
          <motion.figure key={s.src} data-reveal="" variants={revealItem} className={s.big ? "gallery-big" : undefined} style={{ margin: 0, position: "relative", overflow: "hidden", borderRadius: "var(--r-md)", background: "#0a0a09" }}>
